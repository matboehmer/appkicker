...
public class NotifyingAsyncQueryHandler extends AsyncQueryHandler {
    ...
    private Set<Integer> tokensOfRunningQueries;
    ...
    public NotifyingAsyncQueryHandler(ContentResolver resolver, AsyncQueryListener listener) {
        super(resolver);
        tokensOfRunningQueries = new HashSet<Integer>(3);
    ...
    public void startQuery(Uri uri, String[] projection) {
        doStartQuery(-1, null, uri, projection, null, null, null);
    }
    ...
    public void startQuery(int token, Uri uri, String[] projection) {
    	doStartQuery(token, null, uri, projection, null, null, null);
    }
    ...
    public void startQuery(Uri uri, String[] projection, String sortOrder) {
        doStartQuery(-1, null, uri, projection, null, null, sortOrder);
    }
    ...
    public void startQuery(Uri uri, String[] projection, String selection, String[] selectionArgs, String orderBy) {
    	doStartQuery(-1, null, uri, projection, selection, selectionArgs, orderBy);
    }

    private void doStartQuery(int token, Object object, Uri uri, String[] projection, String object2, String[]  object3, String object4) {
    	final boolean queryIsRunning = tokensOfRunningQueries.contains(token);
    	if (!queryIsRunning) {
    		Log.d(Utils.TAG, "starting query for token " + token);
    		tokensOfRunningQueries.add(token);
    		startQuery(token, object, uri, projection, object2, object3, object4);
    	} else {
    		Log.d(Utils.TAG, "query for token " + token + " already running");
    	}
    }
    ...
    protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
    	tokensOfRunningQueries.remove(token);
		Log.d(Utils.TAG, "removed token " + token + " from running queries");
